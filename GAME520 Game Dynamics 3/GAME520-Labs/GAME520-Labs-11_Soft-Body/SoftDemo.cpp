...
static void	Init_ClothAttach( SoftDemo* pdemo )
{
	...
	btSoftBody*		psb = btSoftBodyHelpers::CreatePatch( pdemo->m_softBodyWorldInfo, btVector3( -s, h, -s ),
														  btVector3( +s, h, -s ),
														  btVector3( -s, h, +s ),
														  btVector3( +s, h, +s ), r, r, 1 + 4, true );
	btSoftBody::Material*	pm = psb->appendMaterial( );
	pm->m_flags -= btSoftBody::fMaterial::DebugDraw;
	psb->generateBendingConstraints( 2, pm );
	psb->rotate( btQuaternion( M_PI, M_PI_2, 0 ) );
	psb->translate( btVector3( 0.80f, 1, 4 ) );
	//psb->m_cfg.kLF = 0.05;
	//psb->m_cfg.kDG = 0.01;
	psb->m_cfg.kLF = 0.004;
	...
	psb->setWindVelocity( btVector3( -15, 1, -2 ) );
	pdemo->getSoftDynamicsWorld()->addSoftBody( psb );

	btTransform startTransform;
	startTransform.setIdentity();
	startTransform.setOrigin( btVector3( 5, -5.0f, 0 ) );
	btCollisionShape* pole = new btBoxShape( btVector3( 0.2f, 10, 0.2f ) );
	btRigidBody*		body = pdemo->localCreateRigidBody( 0, startTransform, pole );
