...
static void	Init_ClothAttach( SoftDemo* pdemo )
{
	...
	btSoftBody*		balls[numBalls];

	btVector3 startPos[ numBalls ];

	startPos[ 0 ] = btVector3( 0, 0, 0 );
	startPos[ 1 ] = btVector3( 2, 0, 0 );
	startPos[ 2 ] = btVector3( 0, 0, 2 );

	for ( int i = 0; i < numBalls; i++ )
	{
		btSoftBody*		ball = btSoftBodyHelpers::CreateEllipsoid( pdemo->m_softBodyWorldInfo, startPos[i], btVector3( 1, 1, 1 ), 150 );

		btSoftBody::Material*	ballMat = ball->appendMaterial( );
		ballMat->m_flags -= btSoftBody::fMaterial::DebugDraw;
		ballMat->m_kLST = 1;
		ballMat->m_kVST = 1;
		ball->generateBendingConstraints( 4, ballMat );

		ball->m_cfg.kPR = 0.5f;
		ball->m_cfg.kVC = 1;

		ball->setWindVelocity( btVector3( 0, 5.0f, 0 ) );

		ball->m_cfg.aeromodel = btSoftBody::eAeroModel::V_OneSided;
		ball->setTotalMass( 0.001f );
		ball->translate( btVector3( 0, -8, 0 ) );
		pdemo->getSoftDynamicsWorld( )->addSoftBody( ball );
	}
	...
	{
		startTransform.setIdentity();
		startTransform.setOrigin( btVector3( 4, -8, 0 ) );
		btCollisionShape* boxWall = new btBoxShape( btVector3( 0.1f, 4, 4 ) );
		btRigidBody*		bodyBoxWall = pdemo->localCreateRigidBody( 0, startTransform, boxWall );
	}
	...
	{
		startTransform.setIdentity( );
		startTransform.setOrigin( btVector3( 0, -8, 4 ) );
		btCollisionShape* boxWall = new btBoxShape( btVector3( 4, 4, 0.1f ) );
		btRigidBody*		bodyBoxWall = pdemo->localCreateRigidBody( 0, startTransform, boxWall );
	}
	...
	{
		startTransform.setIdentity( );
		startTransform.setOrigin( btVector3( 0, -3.4f, 0 ) );
		btCollisionShape* boxWall = new btBoxShape( btVector3( 4, 0.1f, 4 ) );
		btRigidBody*		bodyBoxWall = pdemo->localCreateRigidBody( 4, startTransform, boxWall );

		btHingeConstraint* hinge = new btHingeConstraint( *bodyBoxWall, btVector3( 4, 0, 0 ), btVector3( 0, 0, 1 ) );
		hinge->setLimit( -3.14 / 2 - 1, 3.14 / 2 );
