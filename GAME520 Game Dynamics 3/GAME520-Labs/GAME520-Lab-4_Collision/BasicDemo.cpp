...
float fRadius = 1.0f;
const float fCoefficientOfRestitution = 0.5f;
...
void BasicDemo::clientMoveAndDisplay()
{
	...
	for (int i = 0; i < 2; i++)
	{
		for (int j = 0; j < 2; j++)
		{
			if(i != j && j > i)
			{
				if (m_vPosition[i].distance(m_vPosition[j]) < fRadius * 2)
				{
					///////////////////////////////////////////////////////////////////////
					//////////	CALCULATE COLLISION NORMAL	///////////////////////////////
					///////////////////////////////////////////////////////////////////////
					btVector3 normal = btVector3();
					normal = m_vPosition[i] - (m_vPosition[j]);
					normal.normalize();
					float Vr = normal.dot(m_vVelocity[i]-m_vVelocity[j]);

					btVector3 r12 = ((m_vPosition[j] - m_vPosition[i])/2);

					////////////////////////////////////////////////////////////////////////
					//////////  CALCULATE JDENOM1		////////////////////////////////////
					////////////////////////////////////////////////////////////////////////

					btVector3 s1 = r12.cross(normal);

					btVector3 s2 = m_sphereInetiaTensor[i].inverse() * s1;

					btVector3 s3 = s2.cross(r12);

					float jDenom1 = normal.dot(s3);

					////////////////////////////////////////////////////////////////////////
					//////////  CALCULATE JDENOM2		////////////////////////////////////
					////////////////////////////////////////////////////////////////////////

					btVector3 t1 = r12.cross(normal);

					btVector3 t2 = m_sphereInetiaTensor[j].inverse() * t1;

					btVector3 t3 = t2.cross(r12);

					float jDenom2 = normal.dot(t3);

					////////////////////////////////////////////////////////////////////////
					//////////  CALCULATE J				////////////////////////////////////
					////////////////////////////////////////////////////////////////////////

					float J = (-1.0f * Vr * (fCoefficientOfRestitution + 1))/((1/fMass) + (1/fMass) + (jDenom1) + (jDenom2));

					////////////////////////////////////////////////////////////////////////
					//////  CALCULATE Final Velocities	////////////////////////////////////
					////////////////////////////////////////////////////////////////////////

					m_vVelocity[i] = m_vVelocity[i] + (J * normal)/fMass;

					m_vVelocity[j] = m_vVelocity[j] + (-J * normal)/fMass;

					////////////////////////////////////////////////////////////////////////
					//////  CALCULATE Final Angular Velocities	////////////////////////////
					////////////////////////////////////////////////////////////////////////

					btVector3 u1 = r12.cross(J * normal);

					btVector3 u2 = m_sphereInetiaTensor[i].inverse() * u1;

					m_vAngularVelocity[i] = m_vAngularVelocity[i] + u2;

					u1 = r12.cross(-J * normal);

					u2 = m_sphereInetiaTensor[j].inverse() * u1;

					m_vAngularVelocity[j] = m_vAngularVelocity[j] + u2;

					//m_vVelocity[i] -= 2 * (m_vVelocity[j].dot(normal) * normal);
				}
			}
		}
	}
...
void BasicDemo::initPhysics()
{
	...
	// Here are the dynamic bodies
	{
		...
		float start_x[] = {0.0, 1.5}; // collision angle
		...
		for(int i = 0; i < 2; i++)
		{
			...
			m_sphereInetiaTensor[i] = btMatrix3x3();
			m_sphereInetiaTensor[i].setIdentity();
			...
			float xx = (2.0f/5.0f) * fMass * fRadius * fRadius;
			m_sphereInetiaTensor[i].setValue(	xx,	0,	0,
												0,	xx,	0,
												0,	0,	xx);
		}
